...
describe('CartService', () => {
  ...
  describe('#sendComplaint', () => {
    it('should return 10 if client without priority', async () => {
      spectator
        .inject<PriorityClientsService>(PriorityClientsService)
        .isPriority.and.returnValue(false);

      const actual: number = await spectator.service
        .sendComplaint('it does not work!', 'just_a_client')
        .toPromise();
    ...
    it('should not send complain if client without priority', async () => {
      ...
      const complaintService: ComplaintsService &
        SpyObject<ComplaintsService> = spectator.inject<ComplaintsService>(
        ComplaintsService
      );
    ...
    it('should send complain if client is priority', async () => {
      ...
      await spectator.service
        .sendComplaint('it does not work!', 'important_client')
        .toPromise();
